...
export const UserProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const refreshFeed = useCallback(async () => {
        ...
        const username = user.handle.startsWith('@') ? user.handle.substring(1) : user.handle;
                    ...
                    try {
                        ...
                        if (Array.isArray(userData)) {
                            ...
                            .map(p => {
                                ...
                                return {
                                    id: postIdStr, author: { name: user.name, handle: user.handle, avatar: user.avatarUrl },
                                    imageUrl: imageUrl,
                                    videoUrl: videoUrl,
                                    caption: `${p.titulo}\n${p.descrição}`,
                                    likes: parseInt(String(p.likes), 10) || 0, comments: p.comments || 0,
    ...
    useEffect(() => {
        const initialLoad = async () => {
             if (user) {
                ...
                setFeedPosts([]); // Limpa os posts do usuário anterior ao carregar
                await refreshFeed();
    ...
    return (
        <UserContext.Provider value={{ user, updateUser, isLoading, isFeedLoading, feedPosts, updateFeed, togglePostLike, feedError, refreshFeed, isRefreshing }}>
            {children}
