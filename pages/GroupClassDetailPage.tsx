            ...
            <main className="flex-grow overflow-y-auto">
                {groupClass.videoUrl ? (
                    ...
                    <div className="h-56 bg-slate-200 dark:bg-slate-800">
                        <PixabayImage searchQuery={groupClass.title} alt={groupClass.title} className="w-full h-full object-cover" minW={400} minH={224} />
                    </div>
                )}
                        ...
                        <div className="space-y-3">
                            {groupClass.participants.map(p => (
                                <div key={p.id} className="flex items-center space-x-3 bg-white dark:bg-slate-800 p-2 rounded-lg shadow-sm">
                                    <PixabayImage searchQuery={`${p.name} portrait`} alt={p.name} className="w-10 h-10 rounded-full object-cover" minW={40} minH={40} />
